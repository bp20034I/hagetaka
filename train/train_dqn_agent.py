...
# DQNエージェントのトレーニング関数
def train_dqn_agent(agent, env, n_episodes, gamma, num_experiments, target_update_frequency):
    """
    ...
    for experiment in range(num_experiments):
        ...
        for episode in range(n_episodes):
            ...
            while not done:
                action = agent.get_action(state)  # DQNエージェントが行動を選択
...
agent = DQNAgent(state_size, action_size)  # DQNエージェントの初期化

n_episodes = 100000  # トレーニングするエピソード数
gamma = 0.50  # 割引率

# トレーニングを実行
total_rewards = train_dqn_agent(agent, env, n_episodes, gamma, num_experiments=5)
"""
